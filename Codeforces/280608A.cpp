...
int main() {
    ...
    if (c == '1'){
        ...
        if (s.length() == 2){
            cout << cnt(s[0]) + cnt(s[1]) << endl;
            return 0;
        }
        loop(i, 0, s.length() - 2, 3){
            ans += cnt(s[i]);
            ans += cnt(s[i + 1]);
            ans += cnt(s[i + 2]);
        }
        ...
        if (s.length() == 2){
            cout << cnt2(s[0]) * cnt2(s[1]) << endl;
            return 0;
        }
        loop(i, 0, s.length() - 2, 3){
            ans *= cnt2(s[i]) > 0 ? cnt2(s[i]) : 1;
            ans *= cnt2(s[i + 1]) > 0 ? cnt2(s[i + 1]) : 1;
            ans *= cnt2(s[i + 2]) > 0 ? cnt2(s[i + 2]) : 1;
        }
        cout << ans - 1 << endl; 
        }
    }
